...
function ModalDetalle({card,colKey,colStyles,colKeys,onClose,onMove,onComment,onArchive,onRestore,onUpdateResponsables,onAdjuntarArchivo,onEditarPedido,perfil,usuarios=[],grupos=[],esArchivado=false,headerColor='#3a9e3f'}) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'20px 16px',zIndex:100,overflowY:'auto'}} onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div style={{background:'#fff',borderRadius:14,width:'100%',maxWidth:540,marginTop:20,marginBottom:20,overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,0.28)'}}>
...
function ModalNuevo({onClose,onCreate,headerColor='#3a9e3f',usuarios=[],grupos=[]}) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:'20px 16px'}} onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div style={{background:'#fff',borderRadius:14,width:'100%',maxWidth:460,overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,0.28)'}}>
...
function Tablero({tableroId,onVolver,perfil,onLogout}) {
  ...
  useEffect(()=>{
    ...
    function conectar() {
      ...
      ch = sb.channel(`tablero-${tableroId}-${Date.now()}`)
    ...
    function onVisible() { if(!document.hidden) { cargar(); conectar(); } }
  ...
  useEffect(()=>{
    const ch2 = sb.channel('session-watch-'+tableroId)
      .subscribe();
    const check = setInterval(async()=>{
      const {data:{session}} = await sb.auth.getSession();
      if(!session) setSessionExpired(true);
    }, 60000); // check every minute
    return ()=>{ sb.removeChannel(ch2); clearInterval(check); };
  },[]);
...
function CampanaNotif({perfil}) {
  ...
  useEffect(()=>{
    ...
    const ch = sb.channel('notif-'+perfil.id)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'notificaciones',filter:`usuario_id=eq.${perfil.id}`},()=>cargar())
      .subscribe();
